...
triggers:
  - trigger: state
    entity_id:
      - binary_sensor.premium_bonds_this_month
    from: "off"
    to: "on"
actions:
  - action: notify.<your_notifier>
    data:
      ...
      message: >
        {{ state_attr('binary_sensor.premium_bonds_this_month', 'header') }}

        {{ state_attr('binary_sensor.premium_bonds_this_month', 'tagline') }}
```

## Privacy and upgrading

The holder number is stored in the Home Assistant config entry only because it is
required to check results with NS&I. It is not added to entity names, attributes,
or identifiers generated by new installations.

When upgrading, existing entity IDs are deliberately preserved to avoid breaking
automations. If an existing entity ID was originally generated from a holder
number, it may continue to contain that number until you manually rename the
entity in Home Assistant. The integration migrates the corresponding entity
unique ID without renaming the entity ID. Home Assistant records the previous
unique ID in its entity registry metadata as part of that supported migration.
